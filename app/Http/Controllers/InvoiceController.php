        ...
        if(\Auth::user()->can('delete invoice'))
        {
            if($invoice->created_by == \Auth::user()->creatorId())
            {
                ...
                foreach($invoice->payments as $invoices)
                {
                    ...
                    $totalPaidAmount[] = $invoices->amount;
                    $invoices->delete();
                }
                $invoice->delete();
                
                if($invoice->customer_id != 0 && $invoice->status != 4)
                {

                    if($invoice->status == 3){
                        $totalPaidAmount = array_sum($totalPaidAmount);
                        Utility::userBalance('customer', $invoice->customer_id, $invoice->getTotal()-$totalPaidAmount , 'debit');
                    }else{
