            ...
            success: function (file, response) {
              location.reload();
                if (response.is_success) {
                    ...
                    show_toastr('{{__("error")}}', 'This operation is not perform due to demo mode.', 'error');
...
<div class="col-md-6 float-end d-flex col-md-6 text-end d-flex align-items-center justify-content-end mb-4 ">
    @if(\Auth::user()->can('send contract mail') && ($contract->edit_status == 'accept'))
        ...
        @if((\Auth::user()->type =='company') && ($contract->company_signature == '') && ($contract->edit_status == 'accept'))
        <div class="action-btn ms-2">
        ...
        @endif
        @if((\Auth::user()->type != 'company') && ($contract->customer_signature == '') && ($contract->edit_status == 'accept'))
        <div class="action-btn ms-2">
                                                        ...
                                                        <div class="row align-items-center">
                                                            <div class="col-auto">
                                                                ...
                                                                @elseif($comment->type == 'customer')
                                                                    ...
                                                                    <a href="{{ !empty($customer->avatar) ? $profile . $customer->avatar : $profile . '/avatar.png' }}" target="_blank" class="avatar avatar-sm rounded-circle">

                                                                        <img  class="rounded-circle"  width="50" height="50" src="{{ !empty($customer->avatar) ? $profile . $customer->avatar : $profile . '/avatar.png' }}">
                                                                    </a>
                                                                @endif
                                                            </div>
                                                            <div class="col ml-n2">
                                                            ...
                                                            @if((\Auth::user()->type != 'company') && ($contract->edit_status == 'accept') && (\Auth::user()->id == $comment->created_by) && (Auth::guard('customer')->user()->email == \Auth::user()->email))
                                                                <div class="col-auto">
